...
import { RechnungViewDialog } from '@/components/dialogs/RechnungViewDialog';
...
const RECHNUNG_FIELDS = [
  { key: 'rechnungsnummer', label: 'Rechnungsnummer', type: 'string/text' },
  { key: 'rechnungsdatum', label: 'Rechnungsdatum', type: 'date/date' },
  { key: 'betrag', label: 'Betrag (EUR)', type: 'number' },
  { key: 'lieferant', label: 'Lieferant/Aussteller', type: 'string/text' },
  { key: 'kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'buero', label: 'Büromaterial' }, { key: 'it_software', label: 'IT & Software' }, { key: 'reise', label: 'Reisekosten' }, { key: 'marketing', label: 'Marketing' }, { key: 'miete', label: 'Miete & Nebenkosten' }, { key: 'versicherung', label: 'Versicherungen' }, { key: 'sonstiges', label: 'Sonstiges' }] },
  { key: 'rechnungsdatei', label: 'Rechnung hochladen', type: 'file' },
  { key: 'bezahlt', label: 'Rechnung bereits bezahlt', type: 'bool' },
  { key: 'zahlungsdatum', label: 'Zahlungsdatum', type: 'date/date' },
  { key: 'notizen', label: 'Notizen', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'rechnung', label: 'Rechnung', pascal: 'Rechnung' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('rechnung');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'rechnung': return (data as any).rechnung as Rechnung[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'rechnung': return RECHNUNG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'rechnung': return {
        create: (fields: any) => LivingAppsService.createRechnungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateRechnungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteRechnungEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'rechnung' || dialogState?.entity === 'rechnung') && (
        <RechnungDialog
          open={createEntity === 'rechnung' || dialogState?.entity === 'rechnung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'rechnung' ? handleUpdate : (fields: any) => handleCreate('rechnung', fields)}
          defaultValues={dialogState?.entity === 'rechnung' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Rechnung']}
          enablePhotoLocation={AI_PHOTO_SCAN['Rechnung']}
        />
      )}
      {viewState?.entity === 'rechnung' && (
        <RechnungViewDialog
          open={viewState?.entity === 'rechnung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'rechnung', record: r }); }}
        />
      )}
