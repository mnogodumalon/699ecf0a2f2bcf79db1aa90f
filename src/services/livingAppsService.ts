...
import type { Rechnung, CreateRechnung } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getRechnung(): Promise<Rechnung[]> {
    ...
    return enrichLookupFields(records, 'rechnung');
  }
  static async getRechnungEntry(id: string): Promise<Rechnung | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Rechnung;
    return enrichLookupFields([record], 'rechnung')[0];
  }
  static async createRechnungEntry(fields: CreateRechnung) {
    return callApi('POST', `/apps/${APP_IDS.RECHNUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'rechnung') });
  }
  static async updateRechnungEntry(id: string, fields: Partial<CreateRechnung>) {
    return callApi('PATCH', `/apps/${APP_IDS.RECHNUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'rechnung') });
  }
