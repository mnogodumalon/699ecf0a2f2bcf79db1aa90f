...
export function RechnungDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false, enablePhotoLocation = true }: RechnungDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'rechnung');
      await onSubmit(clean as Rechnung['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "rechnungsnummer": string | null, // Rechnungsnummer\n  "rechnungsdatum": string | null, // YYYY-MM-DD\n  "betrag": number | null, // Betrag (EUR)\n  "lieferant": string | null, // Lieferant/Aussteller\n  "kategorie": LookupValue | null, // Kategorie (select one key: "buero" | "it_software" | "reise" | "marketing" | "miete" | "versicherung" | "sonstiges") mapping: buero=Büromaterial, it_software=IT & Software, reise=Reisekosten, marketing=Marketing, miete=Miete & Nebenkosten, versicherung=Versicherungen, sonstiges=Sonstiges\n  "bezahlt": boolean | null, // Rechnung bereits bezahlt\n  "zahlungsdatum": string | null, // YYYY-MM-DD\n  "notizen": string | null, // Notizen\n}`;
      ...
      if (file.type.startsWith('image/') || file.type === 'application/pdf') {
        try {
          ...
          setFields(prev => ({ ...prev, rechnungsdatei: fileUrl }));
        } catch (uploadErr) {
  ...
  const DIALOG_INTENT = defaultValues ? 'Edit Rechnung' : 'New Rechnung';
            ...
            {fields.rechnungsdatei ? (
              <div className="flex items-center gap-3 rounded-lg border p-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm truncate text-foreground">{fields.rechnungsdatei.split("/").pop()}</p>
                  <div className="flex gap-2 mt-1">
                      ...
                      <input
                        ...
                        onChange={async (e) => {
                          ...
                          try {
                            ...
                            setFields(f => ({ ...f, rechnungsdatei: fileUrl }));
                    ...
                    <button
                      ...
                      onClick={() => setFields(f => ({ ...f, rechnungsdatei: undefined }))}
                    >
                      Remove
                    </button>
                  </div>
